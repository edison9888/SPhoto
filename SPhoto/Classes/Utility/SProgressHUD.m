...
- (void)setMode:(RRProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == RRProgressHUDModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateIndicators {
    ...
    else if (mode == RRProgressHUDModeCustomView && self.customView != nil){
...
+ (RRProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	RRProgressHUD *hud = [[RRProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
#if __has_feature(objc_arc)
	return hud;
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[RRProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		RRProgressHUD *HUD = (RRProgressHUD *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"RRProgressHUDViewIsNillException" 
					format:@"The view used in the RRProgressHUD initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = RRProgressHUDAnimationFade;
        self.mode = RRProgressHUDModeIndeterminate;
        self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == RRProgressHUDAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == RRProgressHUDAnimationZoom) {
