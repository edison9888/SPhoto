...
@protocol RRProgressHUDDelegate;

/////////////////////////////////////////////////////////////////////////////////////////////

typedef enum {
    /** Progress is shown using an UIActivityIndicatorView. This is the default. */
    RRProgressHUDModeIndeterminate,
    /** Progress is shown using a MBRoundProgressView. */
	RRProgressHUDModeDeterminate,
	/** Shows a custom view */
	RRProgressHUDModeCustomView
} RRProgressHUDMode;

typedef enum {
    /** Opacity animation */
    RRProgressHUDAnimationFade,
    /** Opacity + scale animation */
    RRProgressHUDAnimationZoom
} RRProgressHUDAnimation;
 ...
 * The RRProgressHUD window spans over the entire space given to it by the initWithFrame constructor and catches all
 ...
 * - RRProgressHUDModeIndeterminate - shows a UIActivityIndicatorView
 * - RRProgressHUDModeDeterminate - shows a custom round progress indicator (MBRoundProgressView)
 * - RRProgressHUDModeCustomView - shows an arbitrary, user specified view (@see customView)
 *
...
@interface RRProgressHUD : UIView {
	...
	RRProgressHUDMode mode;
    RRProgressHUDAnimation animationType;
	...
	id<RRProgressHUDDelegate> __weak delegate;
#elif __has_feature(objc_arc)
	id<RRProgressHUDDelegate> __unsafe_unretained delegate;
#else
	id<RRProgressHUDDelegate> delegate;
#endif
    NSString *labelText;
 ...
 */
+ (RRProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated;



+ (RRProgressHUD *)showWithView:(UIView *)view status:(NSString *) status animated:(BOOL)animated;
/**
...
/**
 * The UIView (i.g., a UIIMageView) to be shown when the HUD is in RRProgressHUDModeCustomView.
...
/** 
 * RRProgressHUD operation mode. Switches between indeterminate (RRProgressHUDModeIndeterminate) and determinate
 * progress (RRProgressHUDModeDeterminate). The default is RRProgressHUDModeIndeterminate.
 *
 * @see RRProgressHUDMode
 */
@property (assign) RRProgressHUDMode mode;

/**
 * The animation type that should be used when the HUD is shown and hidden. 
 *
 * @see RRProgressHUDAnimation
 */
@property (assign) RRProgressHUDAnimation animationType;

/** 
 * The HUD delegate object. If set the delegate will receive hudWasHidden callbacks when the HUD was hidden. The
 * delegate should conform to the RRProgressHUDDelegate protocol and implement the hudWasHidden method. The delegate
 * object will not be retained.
 */
#if __has_feature(objc_arc_weak)
@property (weak) id<RRProgressHUDDelegate> delegate;
#elif __has_feature(objc_arc)
@property (unsafe_unretained) id<RRProgressHUDDelegate> delegate;
#else
@property (assign) id<RRProgressHUDDelegate> delegate;
#endif
...
@protocol RRProgressHUDDelegate <NSObject>
 ...
 */
- (void)hudWasHidden:(RRProgressHUD *)hud;
